...
const WineList: React.FC<WineListProps> = ({ wines, isRestaurantMenu = false }) => {
  ...
  useEffect(() => {
    if (wines.length > 0) {
      ...
      if (expandedWineId === null && newRecommendations.length > 0) {
            ...
            <div key={wine.id} className="transition-all duration-200">
              ...
              {isExpanded && (
                <div className="relative">
                  ...
                  {/* Full wine card */}
                  <WineCard wine={wine} isFeatured={isTopRecommendation} />
                </div>
              )}
            </div>
          );
